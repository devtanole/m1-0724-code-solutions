...
async function readOnce(): Promise<void> {
  const msg: unknown = await read('foo/bar.html');
  console.log(elapsed(), 'readOnce:', msg);
}

async function readSeveral(): Promise<void> {
  const msg1: unknown = await read('foo1/bar.html');
  console.log(elapsed(), 'readSeveral1:', msg1);
  const msg2: unknown = await read('foo2/bar.html');
  console.log(elapsed(), 'readSeveral2:', msg2);
  const msg3: unknown = await read('foo3/bar.html');
